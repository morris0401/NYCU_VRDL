...
class PromptTrainDataset(Dataset):
    ...
    def _get_gt_name(self, rainy_name):
        base_name = os.path.basename(rainy_name)
        prefix, number = base_name.split('-')
        number = number.replace('.png', '')

        gt_name = os.path.join(
            os.path.dirname(os.path.dirname(rainy_name)),
            "gt",
            f"{prefix}_clean-{number}.png"
        )
    ...
    def _get_nonhazy_name(self, hazy_name):
        base_name = os.path.basename(hazy_name)
        prefix, number = base_name.split('-')
        number = number.replace('.png', '')

        gt_name = os.path.join(
            os.path.dirname(os.path.dirname(hazy_name)),
            "gt",
            f"{prefix}_clean-{number}.png"
        )
...
class DerainDehazeDataset(Dataset):
    ...
    def _get_gt_path(self, degraded_name):
        if self.task_idx == 0:
            ...
            gt_name = gt_name.split('-')[0] + '_clean-' + gt_name.split('-')[1]
        elif self.task_idx == 1:
            gt_name = degraded_name.replace("input", "target")
            gt_name = gt_name.split('-')[0] + '_clean-' + gt_name.split('-')[1]
        ...
        #print("Degraded name: ", degraded_name)
        #print("GT name: ", gt_name)
        return gt_name
